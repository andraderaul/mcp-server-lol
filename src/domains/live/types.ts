...
export const GetScheduleInputSchema = z.object({
  language: LanguageSchema.default("en-US").describe(
    "Language code for the schedule response"
  ),
  leagueId: z
    .string()
    .optional()
    .describe("League ID to filter the schedule by"),
});

export const GetLiveMatchesInputSchema = z.object({
  language: LanguageSchema.default("en-US").describe(
    "Language code for the live matches response"
  ),
});

export const GetLeaguesInputSchema = z.object({
  language: LanguageSchema.default("en-US").describe(
    "Language code for the leagues response"
  ),
  region: z
    .string()
    .optional()
    .describe(
      'Filter leagues by specific region (e.g., "AMERICAS", "EMEA", "ASIA")'
    ),
});
...
export const GetEventDetailsInputSchema = z.object({
  eventId: z
    ...
    .max(100, "Event ID too long")
    .describe(
      'Unique identifier of the esports event (e.g., "110947234567890123" or "lck-2025-spring-finals")'
    ),
  language: LanguageSchema.default("en-US").describe(
...
export const GetMatchVODsInputSchema = z.object({
  ...
  language: LanguageSchema.default("en-US").describe(
    "Language code for the VODs response"
  ),
});
